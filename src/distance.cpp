...
int32_t cmp(const void *key, const void *with) {
    return ((vertex *) key)->dist - ((vertex *) with)->dist;
}

void find_distance(int distance_array[21][80], map_t *map, int pcr, int pcc, int costs[10]) {
    for (int r = 0; r < 21; r++) {
        for (int c = 0; c < 80; c++) {
            distance_array[r][c] = INT_MAX;
        }       
    }
    distance_array[pcr][pcc] = 0;
    ...
    while (heap_peek_min(&h)) {
        vertex *v = (vertex *) heap_remove_min(&h);

        if (v->dist > distance_array[v->r][v->c]) {
            free(v);
            continue;
        }
        ...
        for (int dr = -1; dr <= 1; dr++) {
            for (int dc = -1; dc <= 1; dc++) {
                ...
                if (new_dist < distance_array[nr][nc]) {
                    distance_array[nr][nc] = new_dist;
